...
export default function (self) {
  ...
  self.paste = function (event) {
    ...
    var supportedMimeTypes = ['text/plain', 'text/html', 'text/csv'];
    ...
    itemToPaste.getAsString(function (pasteValue) {
      if (pasteType === 'text/html' && isSupportedHtmlTable(pasteValue)) {
        self.pasteData(
          pasteValue,
          pasteType,
      ...
      if (!pasted) {
        if (pasteableItems.length > 1) {
          var itemToPaste1 = pasteableItems[1];
          itemToPaste1.getAsString(function (pasteValue) {
            self.pasteData(
              pasteValue,
              pasteType,
              self.activeCell.rowIndex,
